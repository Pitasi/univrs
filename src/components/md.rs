...
use crate::{images::StaticImg, markdown};
...
pub fn Dialog(
    ...
    let msg_class = dialog_background(&character, &pos)
        + " border border-black px-4 py-2 rounded-md shadow-neu-1 min-w-0 max-w-full";
    let msg = match parse.as_str() {
        "html" => view! { cx,
            <div class=msg_class><p inner_html=msg /></div>
        },
        "text" => view! { cx,
            <div class=msg_class><p>{msg}</p></div>
        },
        "markdown" => view! { cx,
            <div class=msg_class inner_html={markdown::parse(&msg)} />
        },
        _ => panic!(
            "invalid value for 'parse' attribute: {}. Allowed values: html, text, markdown",
            parse
        ),
    };
    ...
    let (first, second) = match pos.as_str() {
        "left" => (icon, msg),
        "right" => (msg, icon),
        _ => panic!("invalid position for dialog: {}", pos),
    };
...
#[component]
fn CharacterAvatar(cx: Scope, character: String) -> impl IntoView {
    match character.as_str() {
        "bulb" => view! { cx,
            <StaticImg path="static/bulb.webp" alt="The bulb" class="w-full h-full object-contain" />
        }.into_view(cx),
        "raisehand" => view! { cx,
