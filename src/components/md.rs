...
#[component]
pub fn Dialog<G: Html>(cx: Scope, props: DialogProps) -> View<G> {
    ...
    let msg = match props.parse.as_str() {
        "html" => view! { cx,
            div(class=msg_class) {
                p(dangerously_set_inner_html=props.msg.clone())
            }
        },
        "text" => view! { cx,
            div(class=msg_class) {
        ...
        "markdown" => view! { cx,
            div(class=msg_class, dangerously_set_inner_html=markdown::parse(&props.msg))
        },
        _ => panic!(
...
#[component]
fn CharacterAvatar<G: Html>(cx: Scope, props: CharacterAvatarProps) -> View<G> {
